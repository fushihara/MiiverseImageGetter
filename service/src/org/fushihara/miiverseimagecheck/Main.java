...
public class Main {
	...
	private int mLatestImageLimit = 0;
	...
	public void loadConfig() throws IOException {
		Properties properties = FU.loadUtf8Properties("/miiverseImageGet.properties");
		// 保存ディレクトリを取得する
		{
			String baseDir = properties.getProperty("miiverse.saveDirectory");
			mSaveAllBaseDirectory = new File(baseDir);
			System.out.println(
					String.format("bundle.miiverse.saveDirectory=%s", mSaveAllBaseDirectory.getCanonicalPath()));
		}
		if (properties.containsKey("miiverse.account.id")) {
			mId = properties.getProperty("miiverse.account.id");
			System.out.println(String.format("bundle.miiverse.account.id=%s", mId));
		}
		if (properties.containsKey("miiverse.account.password")) {
			mPassword = properties.getProperty("miiverse.account.password");
		...
		// httpクライアントの定義
		{
			...
			if (properties.containsKey("http.header.useragent")) {
				userAgent = properties.getProperty("http.header.useragent");
		...
		// 新着画像の取得制限
		{
			String valS = properties.getProperty("miiverse.LatestImageLimit", "0");
			try {
				mLatestImageLimit = Integer.parseInt(valS);
		...
		for (LoadTarget loadTarget : mLoadTarget) {
			try {
				...
				File saveDirectoryYesterday;
				{
					...
					saveDirectoryYesterday = new File(
							new File(new File(mSaveAllBaseDirectory, loadTarget.mTitle), "new"),
							String.format("%1$tF", day.minusDays(1)));
				}
				...
				int result = HttpRequestManager.getLatestImages(mHttpClient, loadTarget.mBaseUrl, loadTarget.mTitle,
						saveDirectoryToday, files, mLatestImageLimit, new String[] { "artwork", "in_game" });
	...
	public void downloadOneTitleAllHotImages(String gameId, String gameTitle) throws IOException {
		...
		while (true) {
			...
			System.out.println(String.format("downloadOneTitleAllHotImages %s(%s) day=%tF finish. %d items.", gameTitle,
					gameId, day, result));
